...
seurat_endo <- subset(
  seurat_stromal,
  fine_cell_type %in% c("Endo_venous", "Endo_arterial", "Endo_lymphatic")
)
seurat_vec <- subset(
  seurat_endo,
  fine_cell_type %in% c("Endo_venous", "Endo_arterial")
)
...
ggsave(
  filename = file.path(fig_dir, "figS9e_top.pdf"),
  plot = merged_plot,
...
ggsave(
  filename = file.path(fig_dir, "figS9e_bottom.pdf"),
  plot = merged_plot,
  width = 10, height = 4, units = "cm"
)

# ---- LEC (Endo_lymphatic) signatures ----
...
# AUCell on all endothelial cells (to include Endo_lymphatic cells)
...
meta_df_lec <- seurat_endo@meta.data[seurat_endo$fine_cell_type == "Endo_lymphatic", ]
...
ggsave(
  filename = file.path(fig_dir, "figS9g.pdf"),
