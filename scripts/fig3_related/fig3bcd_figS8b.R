...
dp <- DotPlot(seurat_t, features = t_fine_markers)
...
t_fine_dotplot <- ggplot(dotplot_df, aes(x = id, y = features.plot)) +
  ...
  theme(
    ...
    axis.text.x = element_text(colour = "black", angle = 60, face = "italic", 
...
ggsave(
  ...
  plot = egg::set_panel_size(
    t_fine_dotplot,
    width = unit(length(t_fine_markers) / 5.5, "cm"),
    height  = unit(length(levels(dotplot_df$id)) / 5, "cm")
  ),
...
pdf(file.path(fig_dir, "fig3c.pdf"), width = 8, height = 4)
Heatmap(
...
meta_df_cd8t <- seurat_t@meta.data[seurat_t$fine_cell_type %in%
                                     c("CD8T_Teff", "CD8T_Temra", "CD8T_Tex"), ]
...
ggsave(filename = file.path(fig_dir, "fig3d.pdf"),
...
ggsave(filename = file.path(fig_dir, "figS8b.pdf"),
