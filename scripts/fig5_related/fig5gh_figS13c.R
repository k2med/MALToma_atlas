...
# ---- Fig 5g: B cell niche distances by subtype ----
plot_df <- subset(
  distance_df,
  barcodes %in% c(
    "B_Mal vs. CD8T_Tex",
...
ggsave(
  filename = file.path(fig_dir, "fig5g.pdf"),
  plot     = egg::set_panel_size(ridge_p, width = unit(1, "cm"), height = unit(1, "cm")),
  width = 10, height = 10, units = "cm"
)

# ---- Fig S13c (left): Endo_venous vs myeloid subsets ----
plot_df <- subset(
  distance_df,
  barcodes %in% c(
    "Endo_venous vs. Neu_G0S2",
    "Endo_venous vs. Mac_MARCO",
    "Endo_venous vs. Mac_SPARC",
    "Endo_venous vs. Mac_CHIT1",
    "Endo_venous vs. Mac_STAB1"
  )
)
...
ggsave(
  filename = file.path(fig_dir, "figS13c_left.pdf"),
  plot     = egg::set_panel_size(ridge_p, width = unit(1, "cm"), height = unit(2, "cm")),
  width = 10, height = 10, units = "cm"
)

# ---- Fig S13c (right): Endo_arterial vs myeloid subsets ----
plot_df <- subset(
  distance_df,
  barcodes %in% c(
    "Endo_arterial vs. Neu_G0S2",
    "Endo_arterial vs. Mac_MARCO",
    "Endo_arterial vs. Mac_SPARC",
    "Endo_arterial vs. Mac_CHIT1",
    "Endo_arterial vs. Mac_STAB1"
  )
)
plot_df$cell_type <- sapply(strsplit(plot_df$barcodes, " "), `[`, 3)
plot_df$cell_type <- factor(
...
ggsave(
  filename = file.path(fig_dir, "figS13c_right.pdf"),
  plot     = egg::set_panel_size(ridge_p, width = unit(1, "cm"), height = unit(2, "cm")),
  width = 10, height = 10, units = "cm"
)
...
target_topics <- c(
  "CD4T_Tfh", "Mac_SPARC", "FDC_CR2",
  "Neu_G0S2", "Mac_MARCO", "Endo_arterial", "Endo_venous"
)
target_genes <- c(
  "CD40LG", "VEGFA"
)
...
cor_plot_df$gene_topic <- factor(cor_plot_df$gene_topic,
                                 levels = c("CD40LG | CD4T_Tfh", "CD40LG | Mac_SPARC", "CD40LG | FDC_CR2",     
                                            "VEGFA | Neu_G0S2", "VEGFA | Mac_MARCO", "VEGFA | Endo_venous", "VEGFA | Endo_arterial"))

cor_plot_df_sub <- subset(
  cor_plot_df,
  reference_topic %in% c("CD4T_Tfh", "Mac_SPARC", "FDC_CR2") & gene == "CD40LG"
)
...
ggsave(
  filename = file.path(fig_dir, "fig5h_CD40LG.pdf"),
...
# ---- Fig 5h (VEGF/endothelium–myeloid axis topics) ----
cor_plot_df_sub <- subset(
  cor_plot_df,
  reference_topic %in% c("Neu_G0S2", "Mac_MARCO", "Endo_arterial", "Endo_venous") & gene == "VEGFA"
)
...
ggsave(
  filename = file.path(fig_dir, "fig5h_VEGFA.pdf"),
  plot     = egg::set_panel_size(
    vegf_dot,
    width  = unit(length(unique(cor_plot_df_sub$gene_topic)) * 0.33, "cm"),
    height = unit(length(unique(cor_plot_df_sub$subtype)) * 0.33, "cm")
