...
# ---- figS13d top (ligand-cell abundance) ----
out_dir_top <- "figS13d_top_"
...
figS13d_top_dotplot <- ggplot(spot_import_df, aes(x = pred_subtype, y = Target)) +
...
ggsave(
  "figS13d_top.pdf", width = 12, height = 10, units = "cm",
  egg::set_panel_size(
    figS13d_top_dotplot,
...
# ---- figS13d bottom (pathway-cell abundance) ----
out_dir_bottom <- "figS13d_bottom_"
...
figS13d_bottom_dotplot <- ggplot(spot_import_df, aes(x = pred_subtype, y = Target)) +
  geom_point(aes(size = Importance, color = Importance)) +
  facet_wrap(~view) +
...
ggsave(
  "figS13d_bottom.pdf", width = 10, height = 10, units = "cm",
  egg::set_panel_size(
    figS13d_bottom_dotplot,
    width  = unit(length(unique(spot_import_df$Predictor)) * 3 * 0.33, "cm"),
    height = unit(length(unique(spot_import_df$Target)) * 0.33, "cm")
  )
)
...
for (slide_id in slide_ids) {
  ...
  invisible(run_colocalization(
    ...
    out_label        = slide_id,
    assay            = "predictions",
    misty_out_alias  = out_dir_S13b
  ))
}

result_filedir <- grep(out_dir_S13b, list.dirs(), value = T)

misty_res <- collect_results(result_filedir)

pdf("figS13b_left.pdf")
mistyR::plot_interaction_communities(misty_res, "intra", cutoff = 0.5)
dev.off()

pdf("figS13b_right.pdf")
