...
plot_prop_bar <- function(df, var, lme_col = "LME_subtype",
  ...
  ggsave(
    ...
    plot = egg::set_panel_size(p, width = unit(1.5, "cm"), height = unit(1, "cm")),
...
plot_prop_bar(
  ...
  out_name = "fig1i_up.pdf",
  x_levels = c("Ocular_adnexa", "Stomach", "Others")
)

# BIRC3_MALT1_group (ordered)
plot_prop_bar(
  df = clinical_tumor_df,
  var = "BIRC3_MALT_fusion",
  fill_values = fusion_col,
  out_name = "fig1i_bottom.pdf",
  x_levels = c("Neg", "Pos")
)

# TNFAIP3_group (ordered)
plot_prop_bar(
  df = clinical_tumor_df,
  var = "TNFAIP3_mutation",
  fill_values = mutation_col,
  out_name = "figS4e.pdf",
  x_levels = c("Neg", "Pos")
)
