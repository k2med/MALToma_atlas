...
seurat_cd4t <- subset(
  seurat_t,
  fine_cell_type %in% c("CD4T_Treg", "CD4T_Tfh", "CD4T_Tn")
)

# ---- compute Treg proportion per sample ----
treg_prop_df <- data.frame(
  Treg_prop = prop.table(
    table(seurat_cd4t$orig.ident, seurat_cd4t$fine_cell_type == "CD4T_Treg"),
    margin = 1
  )[, 2]
)
...
cor_scatter <- ggplot(treg_prop_mp_df, aes(x = MP_7, y = Treg_prop, color = subtype)) +
  ...
  labs(x = "MP7 score", y = "Proportion of CD4T_Treg (%)") +
  theme(
...
ggsave(
  filename = file.path(fig_dir, "fig2g.pdf"),
