...
gmt_path           <- file.path(src_dir, "B_Mal_signatures.gmt")
...
pdf(file = file.path(fig_dir, "fig2e.pdf"), width = 4, height = 5, onefile = F)
print(pfs_plot)
dev.off()
...
cox_results <- lapply(c("B_Mal", names(gene_sets_list)), function(mp_name) {
...
forest_plot <- ggplot(cox_results) +
  ...
  geom_text(aes(x = Variable, y = y, label = p_label),
...
ggsave(
  filename = file.path(fig_dir, "fig2f.pdf"),
