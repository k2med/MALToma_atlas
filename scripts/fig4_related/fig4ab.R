...
seurat_use <- subset(
  seurat_all,
  fine_cell_type %in% c("Fibro_alveolar", "AT1_AGER", "AT2_SFTPC", 
                        "Hepa_ALB", "Endo_aerocyte", "B_Mal", "B_Nor"),
  invert = TRUE
)
...
# ---- heatmap ----
pdf('fig4a.pdf', width = 8, height = 8)
Heatmap(cor_mat,
        name = "Spearman\ncorrelation",
        col = heatmap_col5,
        cluster_rows = TRUE,
...
df_sum <- data.frame(
  CM1 = rowSums(df_prop[, c("cDC1_XCR1", "pDC_CLEC4C", "FDC_CR2", "Mac_SPARC", 
                            "Fibro_CCL19", "CD4T_Tfh", "CD8T_Tex", 
                            "CD4T_Treg", "cDC2_CLEC10A", "Endo_lymphatic")]),
  CM2 = rowSums(df_prop[, c("Mac_MARCO", "Endo_venous", "CD8T_Temra", "Mac_STAB1", 
                            "CD8T_Teff", "Neu_G0S2", "Endo_arterial", "Peri_PDGFRB")])
)
...
ggsave(filename = 'fig4b.pdf', width = 8, height = 5, units = 'cm',
